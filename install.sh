...
echo -E "$CYAN
    ...
    |_| \_|_/_/\_\\___/|___/ |___|_| |_|___/\__\__,_|_|_|\___|_| 
       ...
       ${BLUE} ─── https://github.com/rodeyseijkens/nix-config ─── ${RESET}
"
...
while true; do
    echo -en "${INFO}Choose a ${GREEN}host${RESET} - [${YELLOW}P${RESET}]ingu (NixOS), [${YELLOW}M${RESET}]alus (Darwin): "
    read -n 1 -r
    echo
    ...
    if [[ $REPLY =~ ^[Pp]$ ]]; then
        ...
        echo -e "${ERROR}Invalid choice. Please select 'P' or 'M'."
        continue
    fi
...
## Get the hardware configuration (NixOS only)
if [[ "$HOST" == "pingu" ]]; then
